...
<div class="section-description">
Automated interpretation of current performance,
competitive pressure and recovery momentum.
</div>
        """,
    ...
    if (
        ...
        if performance_gap > 0:
            ...
            performance_headline = (
                f"{performance_gap:.1f} percentage points behind projection"
            )

            performance_text = (
                f"Current achievement is {format_percent(achieved_avg)} "
                f"compared with competitor projection of "
                f"{format_percent(projection_avg)}. "
                f"The business remains behind the competitive benchmark."
            )
            ...
            performance_headline = (
                f"{abs(performance_gap):.1f} percentage points ahead of projection"
            )

            performance_text = (
                f"Current achievement is outperforming competitor projection. "
                f"Achievement stands at {format_percent(achieved_avg)} "
                f"against projection of {format_percent(projection_avg)}."
            )
            ...
            performance_headline = (
                "Performance aligned with projection"
            )

            performance_text = (
                "Achievement and competitor projection are currently aligned."
            )
        ...
        performance_text = (
            "Insufficient achievement or projection data is available."
        )
    ...
    if backlog_percent_avg > 10:
        ...
        pressure_text = (
            f"Average backlog is {format_percent(backlog_percent_avg)} "
            f"with a total gap of {format_number(backlog_units_sum)} units. "
            f"Immediate performance acceleration should be considered."
        )

    elif backlog_percent_avg > 0:

        pressure_class = "gold"

        pressure_headline = (
            "Competitive gap requires attention"
        )

        pressure_text = (
            f"The business remains behind the benchmark by "
            f"{format_percent(backlog_percent_avg)}. "
            f"Current backlog is {format_number(backlog_units_sum)} units."
        )

    else:

        pressure_class = "green"
        ...
        pressure_text = (
            "No material competitive backlog is currently visible "
    ...
    momentum_text = (
        "Select a Business Area with at least two reporting periods "
        "to generate momentum insights."
    )
    ...
    if len(trend_df) >= 2:
        ...
        if momentum_change > 0:
            ...
            momentum_class = "green"

            momentum_headline = (
                f"Achievement improved by {momentum_change:.1f} percentage points"
            )

            momentum_text = (
                f"Latest performance increased from "
                f"{format_percent(previous_achieved)} to "
                f"{format_percent(latest_achieved)}. "
                f"The current trend indicates positive momentum."
            )
            ...
            momentum_headline = (
                f"Achievement declined by {abs(momentum_change):.1f} percentage points"
            )

            momentum_text = (
                f"Latest performance moved from "
                f"{format_percent(previous_achieved)} to "
                f"{format_percent(latest_achieved)}. "
                f"The downward trend should be monitored."
            )
            ...
            momentum_headline = (
                "Performance momentum is stable"
            )

            momentum_text = (
                "Achievement remained unchanged compared with the previous reporting period."
            )


    # ========================================================
    # TARGET RECOVERY INSIGHT
    # ========================================================
    ...
    if target_remaining_sum > 0:
        ...
        recovery_headline = (
            f"{format_number(target_remaining_sum)} units remain to target"
        )

        recovery_text = (
            f"The current recovery requirement is "
            f"{format_number(target_per_day_avg)} units per day. "
            f"Operational focus should remain on maintaining "
            f"the required daily recovery pace."
        )

    else:

        recovery_class = "green"

        recovery_headline = (
            "Target recovery requirement is minimal"
        )

        recovery_text = (
            "No significant remaining target requirement is visible "
            "within the selected reporting period."
        )
    ...
    with i1:
        ...
        st.markdown(
            ...
            f"""
<div class="insight-card {performance_class}">

<div class="insight-title">
01 / PERFORMANCE POSITION
</div>
...
{momentum_text}
    ...
    with i4:
        ...
        st.markdown(
            ...
            f"""
<div class="insight-card {recovery_class}">
